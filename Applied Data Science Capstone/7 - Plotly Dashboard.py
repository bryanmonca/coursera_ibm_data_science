...
def get_scatter_chart(entered_site, values):
    filtered_df = spacex_df
    filtered_df = filtered_df[(filtered_df['Payload Mass (kg)'] >= values[0]) & (filtered_df['Payload Mass (kg)'] <= values[1])]
    if entered_site == 'ALL':
...
# 0 - 2000
# 4000 - 9000
# 5. Which F9 Booster version (v1.0, v1.1, FT, B4, B5, etc.) has 
